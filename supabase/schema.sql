...
CREATE TABLE IF NOT EXISTS public.profiles (
  ...
  jewel_count   INTEGER DEFAULT 0,
  star_count    INTEGER DEFAULT 0,
  sword_power   INTEGER DEFAULT 1,
  player_health INTEGER DEFAULT 100,
  max_health    INTEGER DEFAULT 100,
  unlocked_skins TEXT[] DEFAULT '{"rusty_novice"}',
  active_skin   TEXT DEFAULT 'rusty_novice',
...
-- USER NOTES (for Scribe Chamber / Study Notes tab)
...
CREATE POLICY "Users view own user_notes" ON public.user_notes FOR SELECT USING (auth.uid() = author_id);
CREATE POLICY "Users manage own user_notes" ON public.user_notes FOR ALL USING (auth.uid() = author_id);

-- QUESTS (for Quest Book / To-Do List tab)
...
CREATE POLICY "Users view own quests" ON public.quests FOR SELECT USING (auth.uid() = author_id);
CREATE POLICY "Users manage own quests" ON public.quests FOR ALL USING (auth.uid() = author_id);
